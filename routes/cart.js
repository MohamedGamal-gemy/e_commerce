...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // توليد sessionId جديد إذا لا يوجد userId و sessionId
    if (!userId && !sessionId) {
      sessionId = uuidv4();
      res.cookie("sessionId", sessionId, {
        httpOnly: true,
    ...
    const findCriteria = userId ? { userId } : { sessionId };
    const cart = await Cart.findOne(findCriteria)
      .populate({ path: "items.productId", select: "title price slug" })
      .populate({
        path: "items.variantId",
        select: "color images",
        transform: (doc) => {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const populatedCart = await Cart.findById(cart._id)
      .populate({ path: "items.productId", select: "title slug price" })
      .populate({
        path: "items.variantId",
        select: "color images",
        transform: (doc) => {
          if (!doc) return doc;
          return { ...doc.toObject(), images: doc.images?.length ? [doc.images[0]] : [] };
        },
      });
